...
function showAPILinks(elem) {
    ...
    if (apiToken && ipAddr && port && modKey && panelURL) {
        ...
        var actions = new Array({
            url: 'ogp_api.php?gamemanager/start',
            lang: 'start_server'
        }, {
            url: 'ogp_api.php?gamemanager/stop',
            lang: 'stop_server'
        }, {
            url: 'ogp_api.php?gamemanager/restart',
            lang: 'restart_server'
        });
        ...
        if (hasRcon && hasRcon === 'true') {
            actions.push({
                url: 'ogp_api.php?gamemanager/rcon',
                lang: 'rcon_command_title',
                additional: '&command={YOUR_RCON_COMMAND}'
            });
        }
        ...
        if (isSteam && isSteam === 'true') {
            actions.push({
                url: 'ogp_api.php?gamemanager/update',
                lang: 'get_steam_autoupdate_api_link',
                additional: '&type=steam',
                selected: true
            });
        }
    ...
    if ($("span.versionNumber").hasClass("hide")) {
        ...
        try {
            ...
            if (successful) {
                ...
                $("span.copyVersionResult").fadeIn('fast', function(e) {
                    hideVLength();
                }).delay(500).fadeOut('slow', function(e) {
                    showVLength();
                }).delay(500).fadeIn('slow', function(e) {
                    hideVLength();
                }).delay(500).fadeOut('slow', function(e) {
                    showVLength();
                }).delay(500).fadeIn('fast', function(e) {
                    hideVLength();
                }).delay(2000).fadeIn('fast', function() {
